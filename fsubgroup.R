...
n<-100  # sample size
rangeval<-c(0,1) # interval
mfunctions<-10 # number of functional predictors
...
for(mset in 1:mfunctions){
  ...
  daytemppcaobj <- pca.fd(datafd, nharm=10, growfdPar) #pca
  varsum<-cumsum(daytemppcaobj$varprop)
  nhar<-min(which(varsum>0.95)) # explain 95% variation in x(t)
  group<-c(group,rep(mset,nhar)) #group
  daytemppcaobj <- pca.fd(datafd, nharm=nhar, growfdPar)
  sore_list[[mset]]<-daytemppcaobj$scores #Sore
  pca_list[[mset]]<-predict(daytemppcaobj$harmonics,seq(0,1,by=1/200)) 
}


group<-factor(x=group)
p_group<-group # the group of Sore
...
mysubgroup_function<-function(x,y,max_iter,RELTOL,theta,gamma_mcp,lambda_sub,lambda_group,p_group){
  #x:Sore; y:response; max_iter:the max number of iteration; RELTOL:stopping rule; theta:penalty parameter(\rho);
  #gamma_mcp,lambda_sub,lambda_group:tuning parameter; p_group:group of Sore
  n<-dim(x)[1]    # sample size
  p<-dim(x)[2]    # number of Sore
  
  ST_MCP<-function(deta,lambda,gamma,theta){ #(6)
  ...
  ST<-function(t,lambda){ #soft thresholding rule
...
sub_fit<-mysubgroup_function(Sore,y,max_iter=1e4,RELTOL=1e-5,theta=1,gamma_mcp=3,lambda_sub=0.15,lambda_group=0.05,p_group)
# result
sub_fit$mu
sub_fit$beta
sub_fit$class_group
